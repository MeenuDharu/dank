...
export class CategoriesComponent implements OnInit {
	...
	constructor(private socket: Socket, private router: Router, public userService: UserService, private socialAuthService: AuthService, private apiService: ApiService, private snackBar: SnackbarService,
		private ldScript: LoadscriptService, private _location: Location,  private cookieService: CookieService, location: PlatformLocation, private browserService :UserBrowserService) { 
	...
	ngOnInit() {
	  ...
	  else
	  {
		...
		this.restaurantDetails = JSON.parse(localStorage.getItem('restaurant_details'));
...
newCategory()
{
	...
	userEmailCheck(modalName,otpModal) {
   ...
   //  console.log(element.target.value.length)
	...
	//	this.social_data['mobile'] = this.mobile_num;
	  ...
	  signinVerify(newOTPModal)
	  {
		this.loaderStatus = true;
		const input = <HTMLInputElement>document.querySelector('input[autocomplete="one-time-code"]');
		console.log("otp value.........", input.value)
		if(this.guestLogin)
		{
			this.userService.SIGNUP_OTP_VALIDATE({ customer_id: this.customer_id, otp: this.otpForm.otp }).then((result: any) => {
				if (result.status) {				
